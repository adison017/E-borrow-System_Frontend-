...
import { FaBuilding, FaGripVertical } from 'react-icons/fa';
...
export default function EditRoomDialog({
  ...
  useEffect(() => {
    ...
    setAllImages([...existingImages.map(url => ({ type: 'url', url })), ...newImages]);
  ...
  const handleImageReorder = (fromIndex, toIndex) => {
    ...
    // Update imageUrls and uploadedFiles
    const urls = newImages.filter(img => img.type === 'url').map(img => img.url);
    const files = newImages.filter(img => img.type === 'file').map(img => img.file);
    setImageUrls(urls);
  ...
  const handleRemoveImage = (index) => {
    ...
    if (item.type === 'url') {
      const urlIndex = imageUrls.findIndex(url => url === item.url);
      onRemoveOldImage(urlIndex);
    } else {
      const fileIndex = uploadedFiles.findIndex(file => file === item.file);
              ...
              if (allImages.length === 0) {
                return (
                  <div 
                    ...
                    onClick={() => document.getElementById('edit-room-file-upload').click()}
                  >
                    ...
                    <div 
                      ...
                      onDrop={() => {
                        if (draggedIndex !== null && draggedIndex !== 0) {
                          handleImageReorder(draggedIndex, 0);
                        }
